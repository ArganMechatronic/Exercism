...
ElectionResult& determine_result(std::vector<ElectionResult>& final_count){
    int winner_position{0}, i{0};
    for (const  ElectionResult& candidates : final_count){
        if (candidates.votes > final_count.at(winner_position).votes) winner_position = i;
        i++;
    }
    final_count.at(winner_position).name = "President " + final_count.at(winner_position).name;
    return final_count.at(winner_position);
}
